...
export const AnalysisPanel: React.FC<AnalysisPanelProps> = ({
  ...
  const clearZones = () => setSelectedZoneIds([]);
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Mode Tabs — unified segmented control */}
      <div className="bg-card px-4 pt-2">
        <SegmentedTabs
          activeId={analysisMode}
          onChange={(id) => setAnalysisMode(id as 'single' | 'cross')}
          tabs={
            showCrossSample
              ? [
                  { id: 'single', label: 'Single Sample', icon: BarChart3 },
                  { id: 'cross', label: 'Cross-Sample', icon: FileStack },
                ]
              : [{ id: 'single', label: 'Single Sample', icon: BarChart3 }]
          }
        />
      ...
      {analysisMode === 'cross' && showCrossSample ? (
        ...
        <div className="flex-1 min-h-0 flex">
          {/* Left Sidebar - Zone Selection (kept here so analysis controls live close to the analysis content) */}
          <div className="w-64 border-r border-border bg-card p-4 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              <Label className="font-mono text-xs uppercase tracking-wider text-muted-foreground cursor-pointer">All Data</Label>
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  <button onClick={selectAllZones} className="text-xs text-primary hover:underline font-mono">All</button>
                  <button onClick={clearZones} className="text-xs text-muted-foreground hover:underline font-mono">Clear</button>
                </div>
              </div>
                ...
                <div className="space-y-1 pr-2">
                  {zones.length === 0 ? (
                    <div className="p-3 bg-muted/30 rounded border border-dashed border-border">
                      <p className="text-xs text-muted-foreground font-mono mb-2">No zones created yet.</p>
                      <p className="text-xs text-muted-foreground/70 font-mono">
                        Switch to <span className="text-primary font-semibold">2D Heatmap</span> view and use the lasso or box tool to create zones.
                      </p>
                    </div>
                  ) : (
                    zones.map(zone => (
            ...
            <div className="pt-3 border-t border-border space-y-2.5">
              ...
              {[
                { label: 'Show Violin', value: showViolin, set: setShowViolin },
                { label: 'Show Points', value: showJitter, set: setShowJitter },
                { label: 'B&W Mode', value: blackAndWhite, set: setBlackAndWhite },
                { label: 'Scatter Density', value: showScatterDensity, set: setShowScatterDensity },
                { label: 'Small Dots', value: showSmallDots, set: setShowSmallDots },
              ].map(opt => (
                <div key={opt.label} className="flex items-center justify-between">
                  <Label className="font-mono text-xs cursor-pointer">{opt.label}</Label>
